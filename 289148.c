...
int main(void)
{
	...
	//placement pers 1
	scanf("%d %d %d %d", &GrandTab[0][LIGNEPERS], &GrandTab[0][COLPERS], 
						 &GrandTab[0][LIGNEBUT], &GrandTab[0][COLBUT]);
	GrandTab[0][ETAT] = 1;
	//placement autres pers
	for (k=1;k<nbp;k++){
		scanf("%d %d %d %d", &GrandTab[k][LIGNEPERS], &GrandTab[k][COLPERS],
							 &GrandTab[k][LIGNEBUT], &GrandTab[k][COLBUT]);
		GrandTab[k][ETAT] = 0;
	//test indices
	...
	for (a=1; a<nbp; a++){
		for (k=0; k<a; k++){
			if (GrandTab[k][LIGNEPERS] == GrandTab[a][LIGNEPERS] && 
			    GrandTab[k][COLPERS] == GrandTab[a][COLPERS]){
